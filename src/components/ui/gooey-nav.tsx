...
function Segment({ gap, span, hasSeam, leftFill, rightFill, reduced, radii, className, style, children }: SegmentProps) {
  ...
  const d = useTransform(marginLeft, (value) => neckPath(value, span));
  ...
  return (
    ...
    >
      {hasSeam && (
        <svg aria-hidden width={span} viewBox={`0 0 ${span} ${NECK_H}`} preserveAspectRatio="none" className={cn("pointer-events-none absolute top-0 right-full h-full", BAR_TEXT)}>
          <defs>
...
function NavLabel({ label, href, icon, isActive, size, activeLabelColor, onSelect }: NavLabelProps) {
  ...
  return href ? <Link href={href} {...props}>{icon}{label}</Link> : <button type="button" {...props}>{icon}{label}</button>;
}
...
export function GooeyNav({
  ...
  const fill = (index: number) => index === active ? activeColor : "currentColor";
      ...
      <ul className="flex items-center">
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              <NavLabel {...navItem} isActive={isActive} size={size} activeLabelColor={activeLabelColor} onSelect={() => {
