...
## Corrección de carrito y categoría PC armadas

Se confirmó con lectura del catálogo real que los equipos disponibles tienen un `stock: 0` heredado del borrador del admin. El formulario de equipos controla `estado` (Disponible/Vendido), sin campo para cantidad. Ahora listado, ficha y actualización del carrito respetan ese estado: cero heredado no bloquea un equipo disponible; vendido continúa bloqueado. Se mantienen límites positivos cuando existen y no cambia el control de stock cero en accesorios/componentes. No se modifica ningún registro ni fórmula de precios.

PC armadas se excluye del listado general Equipos y de sus categorías; sigue en su selector superior y URL propia. Verificación: 25 pruebas aprobadas, TypeScript sin errores, lint de archivos afectados sin errores (2 advertencias de imágenes existentes). `scripts/review-equipment-cart.mjs` usa un navegador aislado, admite solo GET a las APIs y comprueba agregado desde listado/ficha, validación del carrito, PC armadas y catálogo idéntico antes/después. No envía pedidos.
